...
export async function POST(request: NextRequest) {
  ...
  const entries = Array.isArray(body.entries) ? body.entries as ParsedAssistantSchedule[] : [];
  if (!entries.length) return NextResponse.json({ error: "Tidak ada jadwal untuk disimpan." }, { status: 400 });
  ...
  const { error } = await supabase.from("assistant_schedule_templates").upsert(rows, {
    onConflict: "assistant_user_id,weekday,start_time,class_label,course_name",
  ...
  return NextResponse.json({ ok: true, imported_count: rows.length });
}
